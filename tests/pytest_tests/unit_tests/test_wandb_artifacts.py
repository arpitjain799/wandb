...
def test_check_available_space_explicit(caplog, tmp_path, mock_disk_usage):
    ...
    assert filesystem.check_available_space(tmp_path, size=100, reserve=100) == 700

    # Test when the remaining space is smaller than the file size (log a warning).
    with caplog.at_level(logging.WARNING):
        assert filesystem.check_available_space(tmp_path, size=500, reserve=100) == 300
    ...
    with pytest.raises(OSError) as e:
        filesystem.check_available_space(tmp_path, size=850, reserve=100)
    assert e.value.errno == errno.ENOSPC

    # Test when there's not enough space for the file itself
    with pytest.raises(OSError) as e:
        filesystem.check_available_space(tmp_path, size=1000, reserve=0)
    assert e.value.errno == errno.ENOSPC
...
def test_check_available_space_implicit(tmp_path, mock_disk_usage):
    ...
    assert filesystem.check_available_space(file_path, reserve=100) == 500

    with pytest.raises(OSError):
        filesystem.check_available_space(file_path, reserve=700)
...
def test_check_available_space_no_file(caplog, tmp_path, mock_disk_usage):
    ...
    assert filesystem.check_available_space(not_a_file, size=300, reserve=100) == 500

    with pytest.raises(ValueError, match="`path` must be a real file"):
        filesystem.check_available_space(not_a_file, reserve=100)
